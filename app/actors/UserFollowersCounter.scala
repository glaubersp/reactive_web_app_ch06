...
import akka.pattern.{pipe, CircuitBreaker}
...
class UserFollowersCounter @Inject()(ws: WSClient, credentials: TwitterCredentials)
    extends Actor
    with ActorLogging {
          ...
          ws.url("https://api.twitter.com/1.1/users/show.json")
            ...
            .map { response =>
              if (response.status == 200) {
                ...
                rateLimit.foreach {
                  case (remaining, reset) =>
                    log.info(
                      "Rate limit: {} requests remaining, window resets at {}",
                      remaining,
                      reset
                    )
                    if (remaining < 50) {
...
object UserFollowersCounter {
  ...
  def props(ws: WSClient, credentials: TwitterCredentials): Props =
    Props(new UserFollowersCounter(ws, credentials))
}
